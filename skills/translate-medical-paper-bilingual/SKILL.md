...
- Produce a glossary with source section, English term, part of speech, verified textbook-style broad IPA, professional Chinese, plain-language Chinese explanation, source sentence, collocation, and translation note. Use the familiar Chinese English-classroom 48-sound notation described in `references/translation-standard.md`; write long vowels with `:` (for example, `/ˈti:tʃə/`), and retain stress marks. For a phrase, use separate **spreadsheet cells** for each word and its own IPA in alternating columns: `Word 1 | IPA 1 | Word 2 | IPA 2 | Word 3 | IPA 3`; expand pairs of columns for longer phrases. Never combine several pronunciations in one cell. Prefer verified British learner-dictionary pronunciations when available. Do not use `NOT_VERIFIED` or invent a pronunciation to fit the textbook chart.
