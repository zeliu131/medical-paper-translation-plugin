...
- Produce a glossary with source section, English term, part of speech, one verified textbook-style broad IPA pronunciation, professional Chinese, plain-language Chinese explanation, source sentence, collocation, and translation note. Use the familiar Chinese English-classroom 48-sound notation described in `references/translation-standard.md`; write long vowels with `:` (for example, `/ˈti:tʃə/`), and retain stress marks. Prefer a verified British learner-dictionary transcription when available. Do not leave a pronunciation cell blank, use `NOT_VERIFIED`, or invent a pronunciation to fit the textbook chart.
