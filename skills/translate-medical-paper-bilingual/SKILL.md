...
- Produce a glossary with the V2 columns and ordering, including one `英文术语` cell containing the intact term and one adjacent `发音` cell. Use the familiar Chinese English-classroom 48-sound notation described in `references/translation-standard.md`; write long vowels with `:` (for example, `/ˈti:tʃə/`), and retain stress marks. For a phrase, verify and transcribe each word separately, then put all complete `/.../` transcriptions in that **single** `发音` cell in original order, separated only by the Chinese ideographic comma `、` (for example, `/ðə/、/ˈju:təraɪn/、/[verified epithelium pronunciation]/`). Do not create `Word 1/IPA 1` columns or a single phrase-wide `/... .../` transcription. Prefer verified British learner-dictionary pronunciations when available. Do not use `NOT_VERIFIED` or invent a pronunciation to fit the textbook chart.
