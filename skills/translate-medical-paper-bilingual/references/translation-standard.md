...
- In the XLSX glossary, use separate **cells**, with alternating columns `Word 1`, `IPA 1`, `Word 2`, `IPA 2`, `Word 3`, `IPA 3`, etc. Put each original written word (including `the`) in its own `Word n` cell, and its independently verified `/.../` pronunciation in the immediately adjacent `IPA n` cell. For example, an entry `the uterine stroma` has the same row laid out as `the | /ðə/ | uterine | [verified IPA for uterine] | stroma | [verified IPA for stroma]`, where each pipe here denotes a **cell boundary, not a character inside any cell**. Dynamically create enough word/IPA column pairs for the longest selected term; leave unused later pairs genuinely empty for shorter terms. Do not put a phrase-wide transcription or multiple pronunciations separated by `|` into a single cell. Count whitespace-separated words after removing surrounding punctuation and verify that the nonempty word/IPA pairs match in number and order; retain hyphenated compounds as one written unit unless a dictionary treats the components separately. Do not silently omit common words.
